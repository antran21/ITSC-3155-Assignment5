...
def update_one_resource(resource_id: int, resource: schemas.ResourceUpdate, db: Session = Depends(get_db)):
    ...
    if resource_db is None:
        raise HTTPException(status_code=404, detail="Resource not found")
    return resources.update(db=db, resource=resource_db, resource_id=resource_id)
...
def update_one_recipe(recipe_id: int, recipe: schemas.RecipeUpdate, db: Session = Depends(get_db)):
    ...
    if recipe_db is None:
        raise HTTPException(status_code=404, detail="Recipe not found")
    return recipes.update(db=db, recipe=recipe_db, recipe_id=recipe_id)
...
def create_order_detail(recipe: schemas.OrderDetailCreate, db: Session = Depends(get_db)):
    return order_details.create_order_details(db=db, order_detail=order_details)
...
def update_one_order_detail(order_detail_id: int, recipe: schemas.OrderDetailUpdate, db: Session = Depends(get_db)):
    ...
    if order_detail_db is None:
        raise HTTPException(status_code=404, detail="OrderDetail not found")
    return order_details.update(db=db, order_detail=order_detail_db, order_detail_id=order_detail_id)
